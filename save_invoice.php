...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // Handle vehicle - do not create new customer, just create new vehicle for existing customer
    $vehicle_id = null;
    $was_selected = !empty($data['vehicle_id']);
    if ($was_selected) {
        $vehicle_id = (int)$data['vehicle_id'];
        // Verify the vehicle still exists
        $stmt = $pdo->prepare('SELECT v.*, c.full_name, c.phone FROM vehicles v JOIN customers c ON v.customer_id = c.id WHERE v.id = ? LIMIT 1');
        ...
        // If any info changed, treat as new vehicle
        ...
        if ($currentName !== $newName || $currentPhone !== $newPhone || $currentPlate !== $newPlate || $currentCar !== $newCar) {
            $vehicle_id = null; // Will create new below
        }
        // No update for existing vehicle
    }
    ...
    if ($vehicle_id === null) {
        ...
        if (trim($data['customer_name']) !== '' && !empty($plateNumber)) {
            if ($was_selected) {
                // Create new vehicle for the same customer since selected was changed
                $stmt = $pdo->prepare('SELECT customer_id FROM vehicles WHERE id = ?');
                $stmt->execute([(int)$data['vehicle_id']]);
                $old_customer_id = $stmt->fetchColumn();
                if ($old_customer_id) {
                    ...
                    error_log("Created new vehicle ID $vehicle_id for existing customer ID $old_customer_id because selected was changed");
                }
            } else {
                ...
                if ($existingVehicle) {
                    ...
                    if ($existingCustomer) {
                        ...
                        // Customer not found, do not create new customer
                        throw new Exception('Customer not found. Please select an existing vehicle or ensure the customer exists in the system.');
                    }
                }
            }
            ...
            throw new Exception('Plate number is required when specifying customer details. Please refresh the page and try again.');
        }
        // If no customer name, vehicle_id remains null (invoice without vehicle)
    }
    ...
    // Fetch vehicle data if vehicle_id is set
    if ($vehicle_id) {
        $stmt = $pdo->prepare('SELECT v.*, c.full_name, c.phone FROM vehicles v JOIN customers c ON v.customer_id = c.id WHERE v.id = ?');
        $stmt->execute([$vehicle_id]);
        $veh = $stmt->fetch();
        if ($veh) {
            $data['customer_name'] = $veh['full_name'];
            $data['phone_number'] = $veh['phone'];
            $data['car_mark'] = $veh['car_mark'];
            $data['plate_number'] = $veh['plate_number'];
            $data['vin'] = $veh['vin'];
            $data['mileage'] = $veh['mileage'];
        }
    }
