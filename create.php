...
// Load oil data for the invoice form
...
// Create a lookup array for oil prices
$oilPriceLookup = [];
            ...
            <form id="invoice-form" action="save_invoice.php" method="post" enctype="multipart/form-data" onsubmit="return handleSave()" role="form" aria-label="Invoice form">
                ...
                <div class="tab-content" id="vehicle-tab">
                    <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <!-- Vehicle Make and Model Selection -->
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <div>
                                    <label for="input_vehicle_make" class="block text-sm font-medium text-gray-700 mb-2">
                                        <i class="fas fa-car-side mr-1"></i>
                                        ავტომობილის მარკა
                                    </label>
                                    <div class="searchable-dropdown">
                                        <input type="text" id="input_vehicle_make" class="w-full border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 p-3 text-base transition" placeholder="მოძებნეთ მარკა..." autocomplete="off">
                                ...
                                <div>
                                    <label for="input_vehicle_model" class="block text-sm font-medium text-gray-700 mb-2">
                                        <i class="fas fa-car mr-1"></i>
                                        მოდელი
                                    </label>
                                    <div class="searchable-dropdown">
                                        <input type="text" id="input_vehicle_model" class="w-full border border-gray-300 rounded-lg shadow-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 p-3 text-base transition" placeholder="ჯერ აირჩიეთ მარკა" disabled autocomplete="off">
                ...
                <div class="tab-content hidden" id="oils-tab">
                    <div class="bg-white p-6 rounded-xl shadow-lg border border-gray-200">
                        ...
                        <div class="overflow-x-auto">
                            <table class="w-full text-sm bg-white rounded-lg overflow-hidden shadow-sm min-w-[800px]">
                                <thead class="bg-gray-100">
                                    <tr>
                                        <th class="px-4 py-3 text-left">#</th>
                                ...
                                <tbody id="oils-table-body" class="divide-y divide-gray-100">
                                    <!-- Oil rows added via JS -->
                                </tbody>
                            </table>
                        </div>

                        <button type="button" onclick="addOilRow()" class="mt-4 flex items-center gap-2 text-sm bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600 transition">
                            <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                            </svg>
                            ზეთის დამატება
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            if (pn) {
                attachTypeahead(pn, './admin/api_customers.php?q=', c => `${c.plate_number} — ${c.full_name}${c.car_mark ? ' — ' + c.car_mark : ''}${c.vin ? ' — VIN:'+c.vin : ''}` , (it) => {
                    ...
                    setVehicleFromCarMark(it.car_mark || '');
            ...
            if (window.serverInvoice) {
                ...
                updateReviewContent(); // Update review tab with loaded data
            }

            // Initialize tabs
            initializeTabs();

            // Load vehicle makes and initialize searchable dropdowns
            loadVehicleMakes();
        });
        ...
        async function setVehicleFromCarMark(carMark) {
            ...
            try {
                ...
                console.error('Error setting vehicle from car mark:', error);
            }
        }
        ...
        function updateReviewContent() {
            ...
            const carMark = makeText && modelText ? `${makeText} ${modelText}` : (makeText || modelText || 'Not specified');
        ...
        function addOilRow() {
            ...
            tr.innerHTML = `
                ...
                <td class="px-4 py-3 text-center">
                    <button onclick="removeOilRow(${oilRowCount})" class="text-red-400 hover:text-red-600 p-2 rounded-full hover:bg-red-50 transition-colors">
                        <svg class="h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                    </button>
                </td>
            `;
        ...
        function updateOilPrice(row) {
            ...
            const qty = parseFloat(row.querySelector('.oil-qty').value) || 0;
            ...
            if (brandId && viscosityId && packageType) {
                ...
                row.querySelector('.oil-unit-price').value = unitPrice.toFixed(2);
                row.querySelector('.oil-total').textContent = total.toFixed(2) + ' ₾';
            } else {
                row.querySelector('.oil-unit-price').value = '0.00';
        ...
        function loadServerInvoice(inv) {
            ...
            if (inv.car_mark) setVehicleFromCarMark(inv.car_mark);
            ...
            (inv.oils || []).forEach(ol => {
                ...
                tr.querySelector('.oil-brand').value = ol.brand_id || '';
                tr.querySelector('.oil-viscosity').value = ol.viscosity_id || '';
                tr.querySelector('.oil-package').value = ol.package_type || '';
        ...
        function updatePreviewData() {
            ...
            // Set car mark from make and model
            const makeText = document.getElementById('input_vehicle_make').value || '';
            const modelText = document.getElementById('input_vehicle_model').value || '';
            const carMark = makeText && modelText ? `${makeText} ${modelText}` : (makeText || modelText || 'Not specified');
            document.getElementById('out_car_mark').innerText = carMark;
        ...
        function showSuggestions(input, suggestions) {
            ...
            container.innerHTML = '';
            const makeText = (document.getElementById('input_vehicle_make')?.value || '').trim();
            const modelText = (document.getElementById('input_vehicle_model')?.value || '').trim();
            const vehicleVal = makeText && modelText ? `${makeText} ${modelText}` : (makeText || modelText || '');
        ...
        function fetchSuggestions(query) {
            ...
            const makeText = (document.getElementById('input_vehicle_make')?.value || '').trim();
            const modelText = (document.getElementById('input_vehicle_model')?.value || '').trim();
            const vehicle = makeText && modelText ? `${makeText} ${modelText}` : (makeText || modelText || '');
