...
<?php if ($server && $serverItems):
    ...
    $computedGrand = $computedPartsAfterGlobal + $computedSvcAfterGlobal;
            ...
            <h3 class="text-sm font-bold mt-4 mb-2">Engine Oils</h3>
            <table class="w-full border-collapse border border-black text-[8px] sm:text-[10px] lg:text-[12px]">
                <thead>
                    <tr class="bg-gray-100 print:bg-gray-100">
                        <th class="border border-black p-0.5">#</th>
                        <th class="border border-black p-0.5">Brand</th>
                        <th class="border border-black p-0.5">Viscosity</th>
                        <th class="border border-black p-0.5">Package</th>
                        <th class="border border-black p-0.5 text-center">Qty</th>
                        <th class="border border-black p-0.5 text-right">Unit Price</th>
                        <th class="border border-black p-0.5 text-right">Disc %</th>
                        <th class="border border-black p-0.5 text-right">Total</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    ...
                    foreach ($oils as $index => $oil):
                        // Get brand and viscosity names
                        $brandName = '';
                        $viscosityName = '';
                        if (!empty($oil['brand_id'])) {
                            $stmt = $pdo->prepare('SELECT name FROM oil_brands WHERE id = ?');
                            $stmt->execute([$oil['brand_id']]);
                            $brand = $stmt->fetch();
                            $brandName = $brand ? $brand['name'] : '';
                        }
                        if (!empty($oil['viscosity_id'])) {
                            $stmt = $pdo->prepare('SELECT viscosity FROM oil_viscosities WHERE id = ?');
                            $stmt->execute([$oil['viscosity_id']]);
                            $viscosity = $stmt->fetch();
                            $viscosityName = $viscosity ? $viscosity['viscosity'] : '';
                        }

                        // Get price
                        $unitPrice = 0;
                        if (!empty($oil['brand_id']) && !empty($oil['viscosity_id']) && !empty($oil['package_type'])) {
                            $stmt = $pdo->prepare('SELECT price FROM oil_prices WHERE brand_id = ? AND viscosity_id = ? AND package_type = ?');
                        ...
                        $qty = (int)($oil['qty'] ?? 1);
                        $discount = (float)($oil['discount'] ?? 0);
                        $discountedPrice = $unitPrice * (1 - $discount / 100);
                        $total = $discountedPrice * $qty;
                        $oilsTotal += $total;
                    ?>
                    <tr>
                        <td class="border border-black p-0.5 text-center"><?php echo $index + 1; ?></td>
                        <td class="border border-black p-0.5"><?php echo esc($brandName); ?></td>
                        <td class="border border-black p-0.5"><?php echo esc($viscosityName); ?></td>
                        <td class="border border-black p-0.5"><?php echo esc($oil['package_type']); ?></td>
                        <td class="border border-black p-0.5 text-center"><?php echo $qty; ?></td>
                        <td class="border border-black p-0.5 text-right"><?php echo number_format($unitPrice, 2); ?> ₾</td>
                        <td class="border border-black p-0.5 text-right"><?php echo $discount > 0 ? number_format($discount, 1) . '%' : ''; ?></td>
                        <td class="border border-black p-0.5 text-right font-semibold"><?php echo number_format($total, 2); ?> ₾</td>
                    </tr>
                    <?php endforeach; ?>
                    <!-- Oils total row -->
                    <tr class="font-bold bg-gray-100 print:bg-gray-100">
                        <td class="border border-black p-0.5 text-right" colspan="7">Oils Total:</td>
                        <td class="border border-black p-0.5 text-right"><?php echo number_format($oilsTotal, 2); ?> ₾</td>
                    </tr>
                </tbody>
            </table>
            <?php endif; ?>
