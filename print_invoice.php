...
// Load oils for printing
$oils = !empty($invoice['oils']) ? json_decode($invoice['oils'], true) : [];
$oilBrandsMap = $pdo->query('SELECT id, name FROM oil_brands')->fetchAll(PDO::FETCH_KEY_PAIR);
$oilViscositiesMap = $pdo->query('SELECT id, viscosity FROM oil_viscosities')->fetchAll(PDO::FETCH_KEY_PAIR);
$oilsTotal = 0.0;
foreach ($oils as &$o) {
    $b = isset($oilBrandsMap[$o['brand_id']]) ? $oilBrandsMap[$o['brand_id']] : '';
    $v = isset($oilViscositiesMap[$o['viscosity_id']]) ? $oilViscositiesMap[$o['viscosity_id']] : '';
    $o['brand_name'] = $b;
    $o['viscosity_name'] = $v;
    // price lookup
    $stmt = $pdo->prepare('SELECT price FROM oil_prices WHERE brand_id = ? AND viscosity_id = ? AND package_type = ? LIMIT 1');
    $stmt->execute([$o['brand_id'], $o['viscosity_id'], $o['package_type']]);
    $priceRow = $stmt->fetch();
...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class="fixed bottom-6 right-6 z-50 print-hidden">
        <a href="create.php" class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 flex items-center gap-2 font-medium">
    ...
    <div class="bg-white border-b border-gray-200 px-4 py-3 print-hidden sticky top-0 z-40">
        <div class="max-w-7xl mx-auto flex items-center justify-between">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="create.php" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors flex items-center gap-2">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                    </svg>
                    New
