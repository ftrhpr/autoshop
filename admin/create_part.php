...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($name === '') {
        ...
        try {
            $stmt = $pdo->prepare("INSERT INTO parts (name, description, default_price, created_by) VALUES (?, ?, ?, ?)");
            $stmt->execute([$name, $description, $price, $_SESSION['user_id']]);
            $success = 'Part created successfully';
        } catch (Exception $e) {
            error_log('create_part.php - Insert error: ' . $e->getMessage());
            $error = 'Unable to save part: ' . $e->getMessage();
        }
    }
}
?>
